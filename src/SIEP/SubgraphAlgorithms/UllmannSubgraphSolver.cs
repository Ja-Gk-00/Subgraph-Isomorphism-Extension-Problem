...
internal sealed class UllmannSubgraphSolver : ISubgraphIsomorphismSolver
{
    ...
    public bool TryFindSubgraph(Graph pattern, Graph target, out Dictionary<int, int> mapping)
    {
        ...
        return MatchRecursive(0, pattern, target, candidates, current, used, ref mapping);
    }
        ...
        ref Dictionary<int, int> final)
    {
        ...
        {
            if (!matrix[depth, j] || used.Contains(j))
                continue;
